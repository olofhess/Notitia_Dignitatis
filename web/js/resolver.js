...
export function resolveDetail(node){
    ...
    const deest=findSequentialDeest(node,indexChapter,chapters);
    if(deest){
        return {
            chapter:deest,
            relation:"deest",
...
function findBestDirect(chapters,sourceKey){
    // First prefer an exact normalized grammatical key.
    // This makes simple cases such as Quaestor -> quaestoris deterministic.
    for(const chapter of chapters){
        const targetKeys=chapterKeys(chapter);
        for(const targetKey of targetKeys){
            if(keysEqual(targetKey,sourceKey)){
                return chapter;
            }
        }
    }
    // If no exact key exists, allow the target to contain extra qualifiers.
    let best=null;
    let bestMatched=0;
...
function findSequentialDeest(node,indexChapter,chapters){
    // [Deest] has no surviving title to match linguistically.
    // Use sequence only as conservative evidence, and ONLY when
    // the corresponding chapter is explicitly titled [Deest].
    const indexEntries=Array.from(indexChapter.children).filter(
        child=>
            child.tagName!=="title" &&
            child.getAttribute("line")
    );
    const position=indexEntries.indexOf(node);
    if(position<0){
        return null;
    }
    const candidate=chapters[position+1];
    if(!candidate){
        return null;
    }
    const title=getDirectTitle(candidate)
        .replace(/\s+/g," ")
        .trim()
        .toLowerCase();
    return /^\[deest\??\]$/.test(title) ? candidate : null;
}
